...
class Video {
    constructor(path, options) {
        ...
        this.ffmpegCommand = new ffmpeg().input(this.path).renice(10)
            .videoCodec('libx264').audioCodec('copy').outputOptions('-c:s', 'copy')
    ...
    _addX264Option(option) {
        this.x264Options += this.x264Options.length ? ':' + option : option;
    }
        ...
        return new Promise(function(resolve, reject) {
            ...
            _.each(_self.metadata.streams, function(stream) {
                ...
                switch (stream.codec_type) {
                    case 'video':
                        if (stream.codec_name === 'h264' && !_self.options.override)
                            return reject(new Error('Already encoded in h264. Skipping... (use the --override flag to encode hevc videos)'));
            ...
            if (_self.options.multiPass > 1) {
                _self._addX264Option('pass=1:stats=' + Path.join(os.tmpdir(), 'x264stats.log'));
            }
        ...
        return new Promise(function(resolve, reject) {
            ...
            for (let preset of _self.options.asPreset.split(':')) {
                ...
                for (let option in ASPreset) {
                    switch (option) {
                        case 'x264Options':
                            _self._addX264Option(ASPreset[option]);
                            break;
                        case 'preset':
    ...
    encode() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (_self.options.accurateTimestamps)
                _self._addX264Option('keyint=' + math.eval(_self.videoStream['avg_frame_rate']).toFixed(0));
    ...
    multiPass() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            let statsLogLocation = Path.resolve(os.tmpdir(), 'x264stats.log');

            if (_self.options.multiPass === _self.pass + 1) {
                _self._addX264Option('pass=2:stats=' + statsLogLocation);
            } else {
                _self._addX264Option('pass=3:stats=' + statsLogLocation);
            }
    ...
    appendStats() {
        ...
        return new Promise(function(resolve, reject) {
            ...
            helpers.initStatsFile(Path.join(process.cwd(), 'h264ize.csv')).then(function(stream) {
